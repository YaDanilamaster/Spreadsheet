...
double ToDouble(const CellInterface::Value& value) {
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        ...
        case BinaryOpExpr::Divide:
            if (rhs == 0/*< std::numeric_limits<double>::epsilon()*/) {
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        return (type_ == UnaryOpExpr::UnaryMinus ? -1 : 1) * operand_->Evaluate(sheet);
    }
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        ...
        if (auto cell = sheet.GetCell(*cell_)) {
            return ToDouble(cell->GetValue());
        }
