...
void PrintSheet(std::unique_ptr<SheetInterface>& sh) {
    sh->PrintTexts(std::cout);
}
...
void TestSetCellPlainFormula() {
    ...
    auto checkCell = [&](Position pos, std::string text) {
        ...
        ASSERT(cell != nullptr);
        ASSERT_EQUAL(cell->GetText(), text);
        auto test = std::get<double>(cell->GetValue());
        bool stop = true;
    };

    sheet->SetCell("B2"_pos, "4");
    sheet->SetCell("A2"_pos, "4");
    checkCell("A1"_pos, "=A2+10/B2");
    checkCell("A3"_pos, "=A1+5");
...
int main() {
    ...
    //RUN_TEST(tr, TestSetCellPlainFormula);
    TestSetCellPlainFormula();
    return 0;
}